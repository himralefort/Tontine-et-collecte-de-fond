...
def index():
    if current_user.is_authenticated:
        # Récupérer uniquement les tontines de l'utilisateur
        ...
        # Récupérer uniquement les campagnes de l'utilisateur
        user_campaigns = FundraisingCampaign.query.filter_by(
            creator_id=current_user.id
        ).order_by(FundraisingCampaign.created_at.desc()).limit(4).all()
        
        return render_template('index_connected.html',
                           user_tontines=user_tontines,
                           user_campaigns=user_campaigns)
    else:
        # Ne pas montrer de tontines/campagnes aux non-connectés
        return render_template('index_disconnected.html')
